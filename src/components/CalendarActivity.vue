<template>
  <div
    ...
    @click="test"
  >
    <div :class="`w-full h-full flex flex-col p-2 rounded bg-cover bg-center ${classes}`" :style="`background-image: url('${image}'); box-sizing: border-box;`">
...
export default {
  ...
  computed: {
    ...
    classes () {
      let classes = ''
      if (this.image) {
        classes += 'text-white text-shadow shadow-md hover:shadow-lg'
        if (this.infinite) {
      ...
      if (this.everyone) {
        classes += ' border-2 border-red-dark'
      }
      return classes
    }
  },
